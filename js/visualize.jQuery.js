...
$.fn.visualize = function(options, container){
	return $(this).each(function(){
		...
		var o = $.extend({
			...
			barGroupMargin: 10,
			barMargin: 0, //space around bars in bar chart (added to both sides of bar)
		...
		function scrapeTable(){
			...
			var tableData = {
				...
				yLabels: function(){
					...
					for(var l in yLabels) yLabels[l] = o.yLabelPre + addCommas_vis(yLabels[l]) + o.yLabelPost;
					return yLabels;
				}			
			};
		...
		var createChart = {
			...
			line: function(area){
				...
				$.each(yLabels, function(i){  
					...
					else if(i== yLabels.length-1){ topOffset = label.height()/-3; }
					label
		...
		//get title for chart
		var title = o.title || self.find('caption').html();
		var title_txt = o.title || self.find('caption').text();
		
		//create canvas wrapper div, set inline w&h, append
		var canvasContain = (container || $('<div class="visualize" role="img" aria-label="Chart representing data from the table: '+ title_txt +'" />'))
			.height(o.height)
			.width(o.width)
			.append(canvas);
		...
		if(o.appendTitle){
			...
			$('<div class="visualize-x-title">'+ o.xTitle +'</div>').appendTo(infoContain);
		}
		...
		if(o.appendKey){
			...
			self.find(selector).each(function(i){
				$('<li><span class="visualize-key-color" style="background: '+dataGroups[i].color+'"></span><span class="visualize-key-label">'+ $(this).html() +'</span></li>')
					.appendTo(newKey);
			});
